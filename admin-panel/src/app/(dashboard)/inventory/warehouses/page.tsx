...
export default function WarehousesPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await apiRequest<Warehouse[]>("warehouses", { token });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        await apiRequest(`warehouses/${editing.id}`, {
          method: "PATCH",
  ...
  const handleDelete = async (id: string) => {
    if (!token || !confirm("Delete this warehouse?")) return;
    try {
      await apiRequest(`warehouses/${id}`, { method: "DELETE", token });
      load();
    } catch (e) {
        ...
        <DataTable<Warehouse>
          ...
          columns={[
            ...
            { key: "address", header: "Address", render: (w) => w.address ?? "—" },
            {
              key: "isActive",
              header: "Active",
              render: (w) => (w.isActive !== false ? "Yes" : "No"),
            },
            {
              key: "actions",
              header: "Actions",
